...
**Note:-**  
In the first example, $(X, Y, K) = (2, 2, 1)$. Each tick allows at most $1$ step because $\min(K, i) = 1$ for all $i \ge 1$. So every odd tick can add at most $1$ to $X$, and every even tick can add at most $1$ to $Y$. To reach $X=2$ needs two odd ticks ($i=1,3$) and to reach $Y=2$ needs two even ticks ($i=2,4$), hence the minimum is $4$ ticks.

In the first example, $(X, Y, K) = (2, 2, 2)$. In $3$ ticks, the maximum possible is:
...
In the first example, $(X, Y, K) = (2, 2, 3)$. In $3$ ticks:
...
In the first example, $(X, Y, K) = (0, 0, 7)$. The robot starts at $(0,0)$, so the minimum number of ticks is $0$.

In the first example, $(X, Y, K) = (2, 0, 1)$. With $K=1$, each odd tick contributes at most $1$ to $X$. Reaching $X=2$ needs two odd ticks ($i=1$ and $i=3$), and the even tick $i=2$ can be skipped by choosing $d=0$, so the minimum is $3$ ticks.

In the first example, $(X, Y, K) = (0, 2, 1)$. With $K=1$, each even tick contributes at most $1$ to $Y$. To reach $Y=2$ needs two even ticks ($i=2$ and $i=4$); the odd ticks can use $d=0$, so the minimum is $4$ ticks.

In the first example, $(X, Y, K) = (3, 3, 2)$. In $4$ ticks:
...
In the first example, $(X, Y, K) = (6, 1, 3)$. In $5$ ticks:
